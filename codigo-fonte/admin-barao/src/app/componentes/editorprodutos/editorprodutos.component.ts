...
export class EditorprodutosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.categService
      .getAllCategorias()
      .subscribe((res) => (this.listaCategorias = res));
      ...
      this.produtoService.recuperarPeloId(Number(id)).subscribe({
        next: (res: Produto) => {
          ...
          if (!this.produto.categoria)
            this.produto.categoria = { id: 0, nome: '' };
    ...
    this.produtoService.uploadFoto(formData).subscribe({
      next: (res: PathDTO) => {
        ...
        this.produto.linkFoto =
          'https://interlocutorily-thermonuclear-toshia.ngrok-free.dev/imagens/' +
          res.pathToFile;
  ...
  public salvarProduto() {
    ...
    request.subscribe({
      next: () => {
        alert(
          `Produto ${this.mode === 0 ? 'inserido' : 'atualizado'} com sucesso!`,
        );
